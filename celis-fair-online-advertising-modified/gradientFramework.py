...
def welfare_adv_user(shift, i, attr):
    a,b=get_shift(shift,i)
    # Get inverse virtual valuation...
    if i == attr:
       v = (x+2)/2
    else:
       v = (x+3)/2
    tmp=v[a:b]*y[attr][i][a:b]
    for j in range(len(shift)):
        if j != i:
            a,b=get_shift(shift,j)
            tmp=tmp*z[attr][j][a:b]
    
    welfare=integrate.simps(tmp, dx=((max_x-min_x)/samples))
    return welfare

def welfare_total(shift, attr):
    return [[welfare_adv_user(shift,i,attr) for i in range(len(shift))] ]
    return sum([welfare_attribute(shift[a].reshape(-1,1),a) for a in range(numAttr)])
...
def GDRevenue(shift):
    ...
    print("Before projection:", q)
    q = projection(q)
    print("After projection:", q)
