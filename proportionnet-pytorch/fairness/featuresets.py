...
def experiment_1(d):
    ...
    return C, scale_distance(generate_distance(ad_feats, u_feats), d)
...
def experiment_3(d):
    C = [torch.tensor([0, 1]), torch.tensor([2])]
    ad_feats = torch.tensor([[0, 1], [1, 0]])
    u_feats = torch.tensor([
        [0, 0],
        [0, 1],
        [1, 0],
